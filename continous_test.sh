...
if [ $# -eq 0 ]; then
  until ack -f --python  ./object_store_abstraction ./tests | entr -d nosetests --rednose; do sleep 1; done
else
  if [ $# -eq 1 ]; then
    echo "Testing ${1}"
    until ack -f --python  ./object_store_abstraction ./tests | entr -d nosetests -a ${1} --rednose; do sleep 1; done
  else
